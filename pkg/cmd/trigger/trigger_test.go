...
func runHelp(t *testing.T, args []string, golden string) {
	...
	f := newFactory(ios)
	cmd := cmdtrigger.NewCmdTrigger(f)
	cmd.SetOut(out)
...
func TestTriggerHelp(t *testing.T)       { runHelp(t, []string{"--help"}, "trigger-help.txt") }
func TestTriggerCreateHelp(t *testing.T) { runHelp(t, []string{"create", "--help"}, "trigger-create-help.txt") }
